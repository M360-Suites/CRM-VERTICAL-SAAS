...
jest.mock('../src/controllers/emailSyncController', () => {
  const { mockControllerStub } = require('./helpers/featureRouteHarness');
  return Object.fromEntries('getGmailAuthUrl handleGmailCallback gmailStatus syncGmailInbox disconnectGmail listSyncedMessages createContactFromSender linkMessageToContact'.split(' ').map((name) => [name, mockControllerStub(name)]));
});
jest.mock('../src/controllers/socialAccountController', () => {
  const { mockControllerStub } = require('./helpers/featureRouteHarness');
  return Object.fromEntries('listSocialAccounts connectSocialAccount handleConnectCallback disconnectSocialAccount'.split(' ').map((name) => [name, mockControllerStub(name)]));
});
jest.mock('../src/controllers/connectionController', () => {
  const { mockControllerStub } = require('./helpers/featureRouteHarness');
  return { getConnectionStatuses: mockControllerStub('getConnectionStatuses') };
});
jest.mock('../src/controllers/emailWriterController', () => {
  const { mockControllerStub } = require('./helpers/featureRouteHarness');
  return { generateEmailHandler: mockControllerStub('generateEmailHandler'), sendEmailHandler: mockControllerStub('sendEmailHandler') };
});
...
describe('integration APIs', () => {
  it.each([
    ...
    expect(response.status).toBe(handler === 'createContactFromSender' ? 201 : 200);
